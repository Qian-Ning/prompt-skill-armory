...
export function defineCommands(ctx, sb) {
    const commands = [
        {
            name: 'armory',
        ...
        },
        {
            ...
            handler: async (invocation) => {
                ...
                if (name === undefined)
                    return err('/armory-enable <skill-name>');
        ...
        },
        {
            ...
            handler: async (invocation) => {
                const name = tokens(invocation.rawInput)[0];
                if (name === undefined)
                    return err('/armory-disable <skill-name>');
        ...
        },
        {
            ...
            handler: async (invocation) => {
                ...
                const name = args[0];
                if (name === undefined)
                    return err('/armory-install <skill-name>');
        ...
        },
        {
            ...
            handler: async (invocation) => {
                const name = tokens(invocation.rawInput)[0];
                if (name === undefined)
                    return err('/armory-uninstall <skill-name>');
        ...
        },
        {
            ...
            handler: async (invocation) => {
                ...
                if (path === undefined)
                    return err('/armory-import <path-to-cordis.patch.yml>');
                try {
        ...
        },
        {
            ...
            handler: async (invocation) => {
                ...
                if (dir === undefined)
                    return err('/armory-skill-dir <absolute-skill-directory>');
                try {
        ...
        },
        {
            ...
            handler: async (invocation) => {
                ...
                if (zip === undefined)
                    return err('/armory-install-zip <absolute-path-to-skill.zip>');
                try {
                    const installed = await sb.installSkillFromZip(zip);
                    if (installed.length === 0)
                        return err('zip contained no entries');
