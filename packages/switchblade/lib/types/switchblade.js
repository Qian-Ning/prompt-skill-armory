...
import { ID_PREFIX, isCommandName, isInstallSkillName, } from "./invariant.js";
import { parsePatch, renderPatch } from "./patch.js";
...
export class Switchblade extends Service {
    ...
    async writePrompts(prompts) {
        await this.settingsService?.mutate(settingsNamespace(SETTINGS_NAMESPACE), [{ op: 'set', path: ['prompts'], value: prompts }]);
    }
    /**
    ...
    async registerManagedSkill(name, description, content) {
        ...
        if (!current.some((s) => s.name === name)) {
            await this.settingsService?.mutate(settingsNamespace(SETTINGS_NAMESPACE), [{ op: 'set', path: ['installedSkills'], value: [...current, definition] }]);
    ...
    async setDefaultProfile(id) {
        ...
        if (service === undefined)
            return;
        await service.mutate(settingsNamespace('agent-presets'), [{ op: 'set', path: ['default'], value: id }]);
    }
    ...
    async registerCommand(definition) {
        ...
        if (!next.some((row) => row.name === definition.name)) {
            await this.settingsService?.mutate(settingsNamespace(SETTINGS_NAMESPACE), [{ op: 'set', path: ['customCommands'], value: [...next, definition] }]);
        }
    }
    ...
    async unregisterCommand(name) {
        ...
        await this.settingsService?.mutate(settingsNamespace(SETTINGS_NAMESPACE), [{ op: 'set', path: ['customCommands'], value: rest }]);
    }
    /**
    ...
    async importBundle(patch) {
        ...
        if (parsed.defaultProfile !== undefined)
            await this.setDefaultProfile(parsed.defaultProfile);
    }
        ...
        try {
            ...
            if (commands !== undefined && typeof commands.register === 'function') {
                for (const row of settings.customCommands) {
                    ...
                    if (!this.registrations.has(id))
                        this.registrations.set(id, commands.register(passthroughCommand(row)));
                }
            }
        }
        catch (error) {
    ...
    async setOwnedSkill(id, def, enabled) {
        ...
        await this.settingsService?.mutate(settingsNamespace(SETTINGS_NAMESPACE), [{ op: 'set', path: ['installedSkills'], value: next }]);
    ...
    async setOwnedSkillEnabled(id, enabled) {
        ...
        if (definition === undefined)
            throw new Error(`owned skill "${id}" is not installed`);
        const existing = this.registrations.get(id);
        if (existing !== undefined) {
            existing();
            this.registrations.delete(id);
    ...
    agentPresetsOrThrow() {
        ...
        if (service === undefined)
            throw new Error('switchblade requires the agent-presets service to manage prompt profiles');
        return service;
    }
    authorableRoster() {
...
function passthroughCommand(row) {
    if (typeof row.handler === 'function')
        return row;
    return {
        ...row,
        handler: async () => ({ kind: 'success', text: row.description }),
    };
}
export default Switchblade;
//# sourceMappingURL=switchblade.js.map