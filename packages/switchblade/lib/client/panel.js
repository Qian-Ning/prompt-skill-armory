 ...
 */
export function mountSwitchbladePanel(proxy, host) {
    ...
    const groups = {};
    const render = () => {
        for (const element of Object.values(groups))
            element.remove();
        Object.keys(groups).forEach((key) => delete groups[key]);
        void proxy.catalog(undefined).then((raw) => {
            const catalog = raw;
            const counts = catalog.entries.reduce((acc, entry) => {
            ...
            const order = ['skill', 'profile', 'command'];
