...
import mountSwitchbladePanel from './panel.ts'
...
export interface SwitchbladeRemote {
  ...
  listSkills(): Promise<readonly { id: string; name: string; description: string }[]>
  installSkill(input: unknown): Promise<{ id: string; state: string }>
  uninstallSkill(name: string): Promise<{ id: string; state: string }>
  setSkillEnabled(name: string, enabled: boolean): Promise<{ id: string; state: string }>
...
declare module '@deepseek-ai/cordis' {
  interface Context {
    remote: { switchblade: SwitchbladeRemote }
  }
}
 ...
 */
export function switchbladePanel(ctx: Context, host?: HTMLElement): (() => void) | undefined {
