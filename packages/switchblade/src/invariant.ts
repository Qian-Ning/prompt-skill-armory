...
export const MANAGED_KINDS: readonly ManagedKind[] = ['skill', 'profile', 'command']

/** Prefix used to namespaced-entry `id`s at the action seam. */
export const ID_PREFIX: Record<ManagedKind, string> = {
  skill: 'skill',
  profile: 'profile',
  command: 'command',
}
 ...
 */
export function isEntryId(value: string): value is `${ManagedKind}:${string}` {
  const slash = value.indexOf(':')
  if (slash <= 0 || slash === value.length - 1) return false
  const kind = value.slice(0, slash) as ManagedKind
 ...
 */
export function isInstallSkillName(name: string): boolean {
  // Mirrors the SKILL_NAME grammar of @deepseek-ai/dsh-skill.
